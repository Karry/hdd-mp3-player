void Menu(void){
	...
	if (ZmenaPolozky==1){
		...
		sloupec=10;		
		if (PolozkaMenu==0){
				str2ram (StringBuffer, "\x06výbìr souboru   \x07");		
				PisMale(StringBuffer,1,1);
		}else{
				str2ram (StringBuffer, " výbìr souboru    ");		
				PisMale(StringBuffer,0,0);
		}
		radek=3;
		sloupec=10;		
		if (PolozkaMenu==1){
				str2ram (StringBuffer, "\x06možn. pøehrávání\x07");		
				PisMale(StringBuffer,1,1);
		}else{
				str2ram (StringBuffer, " možn. pøehrávání ");		
				PisMale(StringBuffer,0,0);
		}
		radek=4;
		sloupec=10;		
