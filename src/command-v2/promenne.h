...
#define	LCDCS2		PORTAbits.RA1		// výběr řadiče pro pravou část LCD (CS2=1 - vybráno)
#define	LCDCS1		PORTAbits.RA2		// výběr řadiče pro levou část LCD (CS1=1 - vybráno)
#define	LCDE			PORTAbits.RA3		// hodinový signál
#define	LCDRW			PORTAbits.RA4		// volba mezi: RW=0 - zápisem, RW=1 - čtením
...
unsigned char RezimPrehravani=0;		// 0=prohledávání, hrát porád dál; 1=přehrát složku; 2=repeat složky; 3=přehrát jednu; 4=repeat souboru
...
unsigned char VypinatDisk=1;	// po jaké době nečinosti (minuty) se má vypínat disk        (5, 10, 15, 30,60,nikdy)
unsigned char ZtlumitPodsvetleni=5;	// po jaké době bez stisku klávesy ztlumit podsvětlení (5s,10s,30s,1M,5M,nikdy)

unsigned int Odpocet=0xFFFF;	// do této proměnné se při stisku klávesy umístí konstanta v závislosti na ZtlumitPodsvetleni
															// každých 5ms (přerušení) se od této proměnné jednotka odečte (pokud není ZtlumitPodsvetleni==5)
															// pokud se rovná nule, tak se odečítá od AktualniPodsvetleniXXX než dosáhne min. hodnoty...
...
// behove prommene
unsigned char radek, sloupec;	// aktuální pomyslný kurzor. radek může být 0..7, sloupec 0..127
